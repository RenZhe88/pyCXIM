...
def calibration():
    ...
    if not Calibinfor.section_exists('General Information'):
        Calibinfor.init_beamtime('id05_6c', path, detector, pathmask=pathmask)

    if Calibration_type == 'detector':
        Calibinfor.detector_calib(sample_name, scan_num)
        ...
        Calibinfor.single_Bragg_peak_tilt_cal(sample_name, scan_num, peak, error_source, known_error_values)

    elif Calibration_type == 'multiple Bragg 6C':
        Calibinfor.multi_Bragg_peak_U_matrix_cal(sample_name, scan_num_ar, peak_index_ar)
