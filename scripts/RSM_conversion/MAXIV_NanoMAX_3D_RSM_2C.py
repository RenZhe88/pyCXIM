...
def BCDI_preparation():
    ...
    # read the phi values
    theta = scan.get_scan_data('gonphi')
    ...
    if len(wxy) == 4:
        q_origin, new_shape, RSM_unit = RSM_converter.cal_q_range([(pch[1] - wxy[0]), (pch[1] + wxy[1]), (pch[2] - wxy[2]), (pch[2] + wxy[3])], rebinfactor=rebinfactor)
