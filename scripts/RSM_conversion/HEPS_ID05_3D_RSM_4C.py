...
def HEPS_ID05_RSM_4C():
    start_time = time.time()
    # %%Inputs: general information
    year = "2026"
    sample_name = r"BFO20251212_1"
    scan_num = 38
    detector = 'pilatus'
    ...
    # Inputs: paths
    path = r"F:\Work place 4\sample\XRD\Additional Task\20260325 ID05 RSM\Chenchao"
    pathsave = r"F:\Work place 4\sample\XRD\Additional Task\20260325 ID05 RSM\Chenchao"
    ...
    eta = scan.get_scan_data('th')
    scan_step = (eta[-1] - eta[0]) / (len(eta) - 1)
    delta = scan.get_motor_pos('tth')
    chi = scan.get_motor_pos('chi') + 90
    phi = scan.get_motor_pos('phi')
    nu = scan.get_motor_pos('tthh')
    mu = scan.get_motor_pos('thh')
    ...
    if os.path.exists(pathcalib):
        calibinfor = InformationFileIO(pathcalib)
        calibinfor.infor_reader(pathcalib)
    ...
    dataset, mask3D, pch, roi = scan.load_rois(roi=roi, show_cen_image=(not os.path.exists(pathinfor)), normalize_signal=None, correction_mode='constant')
    ...
    print("peak at eta = %.2f, delta = %.2f, chi = %.2f, phi = %.2f, nu = %.2f, energy = %.2f" % (eta[pch[0]], delta, chi, phi, nu, energy))
    ...
    if save_full_3D_RSM:
        ...
        filename = "%s_%05d_RSM_mask.npz" % (sample_name, scan_num)
