...
def BCDI_preparation():
    ...
    year = "2026"
    beamtimeID = "PLT-26-00571-ID05-01"
    sample_name = 'Sample1_1'
    scan_num = 191
    detector = 'e4m'
    ...
    detector_distance = 1218.9774651539371
    ...
    scan = HEPSTifImporter('id05_6c', path, sample_name, scan_num, detector, pathsavefolder, pathmask)
    print(scan)
    ...
    dataset, mask3D, pch, wxy = scan.load_images(roi, wxy, show_cen_image=(not os.path.exists(pathinfor)), normalize_signal=None, correction_mode='constant')
    scan.write_scan()

    scan_npoints = scan.get_num_points()

    # load the scan motors
    if geometry == 'out_of_plane':
        # read the omega values for each step in the rocking curve
        theta = scan.get_scan_data('eta')[:scan_npoints]
