...
def Desy_P10_RC2RSM_6C():
    ...
    if geometry == 'out_of_plane':
        omega = scan.get_scan_data('om')
        phi = scan.get_motor_pos('phi')
        scan_step = (omega[-1] - omega[0]) / (len(omega) - 1)
    ...
    if os.path.exists(pathcalib):
        ...
        para_name_list = [
            'cch', 'distance', 'pixelsize', 'det_rot', 'additional_rotation_matrix']
        print('Could not find the desired scan parameter file! Generate the file with desired parameters!')
        pathcalib = os.path.join(pathsave, 'calibration.txt')
        calibinfor = InformationFileIO(pathcalib)
        calibinfor.gen_empty_para_file(para_name_list, 'Detector calibration')
    ...
    RSM_converter = RSM_6C('RC', omega, delta, chi, phi, gamma, mu, energy,
    ...
    if geometry == 'out_of_plane':
        print("peak at omega = %.2f, delta = %.2f, chi = %.2f, phi = %.2f, gamma = %.2f, mu = %.2f" % (omega[pch[0]], delta, chi, phi, gamma, mu))
    elif geometry == 'in_plane':
        print("peak at omega = %.2f, delta = %.2f, chi = %.2f, phi = %.2f, gamma = %.2f, mu = %.2f" % (omega, delta, chi, phi[pch[0]], gamma, mu))
    ...
    if geometry == 'out_of_plane':
        infor.add_para('omega', section_ar[2], omega[pch[0]])
