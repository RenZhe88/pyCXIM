...
def calibration():
    ...
    # Inputs:Simple calibration with symmetric diffraction peak
        ...
        peak = np.array([4, 2, 0], dtype=float)
        ...
        known_error_values = np.array([0, 0, -0.21401377565661428, 0, 0, 0], dtype=float)
    ...
    # Inputs:Simple calibration with symmetric diffraction peak
        ...
        scan_num_ar = [1, 7]
        geometry = 'out_of_plane'
        peak_index_ar = np.array([[2, 2, 0], [3, 3, 2]], dtype=float)
        surface_dir = np.array([1, 1, 0], dtype=float)
        inplane_dir = np.array([0, 0, 1], dtype=float)
        lattice_constants = [5.44, 5.71, 7.89, 90, 90, 90]
    ...
    if Calibration_type == 'detector':
        ...
        print(cal_q_error_multiple_peak(U_matrix_angles.x, B_matrix, peak_index_ar, q_ar))
        print('expected_UB')
        ...
        pixel_position, position_parameters, calibration_parameters = load_peak_infor(path, p10_file, 19, pathinfor, pathmask, geometry='out_of_plane', detector='e4m')
        q_vector = cal_abs_q_pos(pixel_position, position_parameters, calibration_parameters)

        inv_U = np.linalg.inv(U_matrix)
        inv_B = np.linalg.inv(B_matrix)
        q_hkl = np.dot(inv_B, np.dot(inv_U, np.flip(q_vector)))
        print(q_hkl)
