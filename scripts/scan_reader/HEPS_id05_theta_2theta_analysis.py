...
def theta2theta_analyser():
    ...
    Functions_selected = ['Check mask', 'Theta2theta','Check results']
    ...
    pathsavefolder = r"F:\Work place 4\sample\XRD\Additional Task\20260325 ID05 RSM\raw\Zhanglinxing"
    path_calib = r'F:\Work place 4\sample\XRD\Additional Task\20260325 ID05 RSM\raw\Zhanglinxing\calibration.txt'
    ...
    data, mask_3D, pch, roi = scan.load_rois(normalize_signal=None, correction_mode='constant')
    del mask_3D, pch, roi
    ...
    if 'GIF' in Functions_selected:
        ...
        pathsavegif = os.path.join(scan.get_pathsave(), "%s_%05d.gif" % (sample_name, scan_num))
    ...
    delta = delta_pos[0]
    detector2q = det2q_2D([delta, nu, energy], [distance, pixelsize, det_rot, cch], detector_size)
