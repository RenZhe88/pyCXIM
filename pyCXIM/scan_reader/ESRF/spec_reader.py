...
class ESRFScanImporter(GeneralScanStructure):
    """
    Read and write the spec files generated at ESRF, beamlines.

    Parameters
    ----------
    beamline : str
        The name of the beamline. Now only 'ID01' and 'BM32' is supported.
    path : str
    ...
    def spec_reader(self):
        ...
        pattern5 = r'#L (.+)\n'
        pattern6 = r'#L .+\n([^#]+\n)(?:#C|\n)'
        pattern7 = r'^#D (\w+ \w+ \d+ [\d|\:]+ \d+)'
        pattern8 = r'#UMONO .+nrj=(\d+\.\d+)keV.+\n'
        ...
        self.start_time = re.findall(pattern7, scantext)[0]
        ...
        energy = float(re.findall(pattern8, scantext)[0]) * 1000.0
